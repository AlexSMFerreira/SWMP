...
class HorizonMasker:
    ...
    def __init__(self, fallback_crop_pct=0.40, horizon_margin_pct=0.03):
    ...
    def compute_mask(self, img_bgr):
        ...
        return self._sky_mask(img_bgr.shape, masked), source
    ...
    def make_debug_image(self, left_bgr, source):
        """Draws the yellow raw-Hough horizon + red nudged mask boundary on a copy
        of the left image. Call after compute_mask() for the same frame. Returns a
        BGR image (wrap with make_color_msg to publish)."""
        if self._cur_raw is None or self._cur_masked is None:
            return None
        debug = left_bgr.copy()
        w = debug.shape[1]

        pt_l, pt_r = self._endpoints(self._cur_raw[0], self._cur_raw[1], w)
        cv2.line(debug, pt_l, pt_r, (0, 255, 255), 1, cv2.LINE_AA)        # yellow = raw

        pt_ln, pt_rn = self._endpoints(self._cur_masked[0], self._cur_masked[1], w)
