...
class PointCloudNode(Node):
    def __init__(self):
        ...
        # Setup exact time synchronizers for the color frame and disparity map
        self._sub_left = message_filters.Subscriber(self, Image, p('left_rect_topic').value, qos_profile=sub_qos)
        self._sub_disp = message_filters.Subscriber(self, Image, p('disp_raw_topic').value, qos_profile=sub_qos)

        self._sync = message_filters.TimeSynchronizer(
            [self._sub_left, self._sub_disp],
            queue_size=10
        )
    ...
    def _cb_images(self, left_msg: Image, disp_msg: Image):
        ...
        # 6. Construct structured array matching PointCloud2 memory layout (X, Y, Z, RGB)
        ...
        # 7. Construct final ROS 2 PointCloud2 Message
        msg = PointCloud2()
